---
name: skill-reviewer
description: Fresh-context adversarial reviewer for a SKILL.md in this Agent Skills collection (and its bundled scripts/references/assets). Use after drafting or editing a skill, before publishing a new one, or when a skill has started misfiring or drifting from the CLI/framework/API it documents.
...
4. **Freshness handling.** If the skill states specific facts about a
   ...
   (the Gitea/`tea` maintainers, GitHub's `gh` CLI, npm, etc.) can change
   without notice, check whether it either (a) ships a `scripts/check-*`-style
   or live-fetch script, or (b) explicitly tells the reader to verify against a linked
